//! 
//! A simple and fast random number generator based on [PCG XSH RR 64/32](https://www.pcg-random.org/pdf/hmc-cs-2014-0905.pdf),
//! including a custom `Instant` implementation compatible with WASM.
//!
//! # Examples
//!
//! Flip a coin:
...
//!
//! let v: Vec<i32> = repeat_with(|| mana::rand::i32(..)).take(10).collect();
//! let s: String = repeat_with(mana::rand::alphanumeric).take(10).collect();
//! ```
//!
...
/// Custom `Instant` implementation for non WebAssembly targets
...
/// Custom `Instant` implementation for WebAssembly, using `wasm-bindgen`
#[cfg(target_arch = "wasm32")]
#[derive(Clone, Copy, Debug, PartialEq, Eq, PartialOrd, Ord, Hash)]
...
impl Clone for Rng {
    ...
    /// let base1 = mana::rand::Rng::new();
    /// base1.seed(0x4d595df4d0f33173);
    /// base1.bool(); // Use the generator once.
    ///
    /// let base2 = mana::rand::Rng::new();
